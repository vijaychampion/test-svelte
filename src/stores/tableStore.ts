...
export function createTableStore(initialData, keyField: string) {
  const dispatcher = createEventDispatcher()

  const sortedData = writable([...initialData])
  const selectedRows = writable<Set<any>>(new Set())
  const selectAll = writable(false)
  const expandedRows = writable<Set<any>>(new Set())
  let draggedRowIndex: number | null = null
  let sortOrders: Record<string, 'asc' | 'desc'> = {} // Track sort order for each column
  ...
  const toggleRowSelection = (rowKey: any) => {
  ...
  const toggleRowSelectionRadio = (rowKey: any) => {
    selectedRows.update((current) => {
