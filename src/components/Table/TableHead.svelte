...
<thead class={cn('text-xs text-gray-700 uppercase bg-white', {
  'mobile-hide': screenSize.isMobile,
}, theadStyle)}>
  <tr id="header-row">
    {#if hasCheckBox}
      <th class={cn('!p-4 fixed-width', thStyle)} style="width: 30px;">
    ...
    {/if}
    {#if isDraggable}
      <th class={cn('!p-4 fixed-width', thStyle)} style="width: 30px;">No. </th>
    {/if}
    {#each columns as column, index}
      <th
        ...
        scope="col"
        class={cn(
          'px-6 py-4 fixed-width cursor-pointer',
          {
