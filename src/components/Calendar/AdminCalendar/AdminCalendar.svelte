...
<div
  class="max-w-[75rem] mx-auto w-full h-[80vh] sm:h-screen overflow-y-auto overflow-hidden border border-stroke-Primary rounded-lg bg-BG-Primary"
>
  <header
  ...
  <main class="px-2 py-4 max-w-6xl mx-auto">
    ...
    {:else if $currentView === "Week"}
      ...
      {:else}
        <WeeklyView selectedDate={$selectedDate} currentevents={events} />
      {/if}
    {:else if $currentView === "Month" || ($currentView === "Quarter" && $isMobile)}
      {#if $loading}
